...
<?php $query = "SELECT * FROM cwoptions WHERE type='product' AND active='1' AND trash='0'"; 
$result = mysql_query($query) or die(mysql_error());
while($row = mysql_fetch_array($result)){
$row = PbUnSerial($row);
...
<div class="panel-heading">
<h4 class="panel-title">
<a data-toggle="collapse" data-parent="#accordion" href="#CwDate">
<i class="fa"></i>Release Date</a>
</h4></div>
<div id="CwDate" class="panel-collapse collapse in">
<div class="form-group">
<input type="date" name='releasedate' class="form-control" value='<?php echo $Article['content']['releasedate']; ?>'>
</div></div>
</div>
...
<input type="hidden" name="imgtype" value="portfolio">
