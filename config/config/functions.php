...
function InstagramLatest($Content,$Array){
    $WebId = $Array['webid'];
    $InstagramInfo = $Array['InstagramInfo'];
    if($Content == "" OR $Content == "default"){
        $Social_Url = $Array['siteinfo']['other']['socialauth']['instagram'];
        if($Social_Url == ""){
            $Social_Url = $Array['siteinfo']['other']['socialmedia']['instagram'];
        }
    }else{
        $Social_Url = $Content;
    }
    ...
    $access_token = $row["info"]["access_token"];
    if($access_token == ""){
        $access_token = $row["other"]["auth"];
    }
    if($display_size == ""){
...
function PbFilter($Content){
    ...
    if(is_array($Content["other"])){
        ...
        if($Content["type"] == "post-artist"){
            $CwDb = CwDb();
            $Artist = $Content["id"];
            $query = "SELECT * FROM articles WHERE type='social' AND active='1' AND trash='0' AND webid='$WebId' AND other LIKE '%-" . $Artist . "-%'";
            $result = mysqli_query($CwDb, $query);
            while($row = mysqli_fetch_assoc($result)){
                $SocialCategory = $row["category"];
                $Content["$SocialCategory"] = $row["url"];
            }
        }
    }
...
function CwGallery($Array,$files){
    ...
    if($files['gallery']){
		...
		($file["type"] == "image/jpeg") ||
	        ($file["type"] == "image/png") ||
	        ($file["type"] == "image/jpg")){
                    $Gallery_Img = "./uploads/$DomainFolder/images/$Rand" . $file['name'];
                    $Images = $Array['siteactual']['domain'] . "/uploads/$DomainFolder/images/$Rand" . $file['name'];
                    if($file['name'] != ""){
                        ...
                        mysqli_query($CwDb,"INSERT INTO images (img, album, webid) VALUES('$Images', '$Article_Id', '$WebId' ) ") or die(mysql_error());
                    }
                }
            }
        }
            ...
            if($file["error"] > 0){
				...
				($file["type"] == "image/jpg")){
                    ...
                    if($file['name'] == ""){ 
                        ...
                        $Article_Img = "http://$_SERVER[HTTP_HOST]/uploads/$DomainFolder/images/$Rand" . $file['name'];
                        $filename = compress_image($file['tmp_name'], $Gallery_Img, 90);
                        $Img["loc"] = "uploads/$DomainFolder/images/$Rand" . $file['name'];
                    }
                }
            }
        }
...
function CwMediaFile($Array,$files,$Rand){
    ...
    if($files['mediafile']){
        foreach($files['mediafile'] as $file){
			...
			$file['name'] = $resizedFile = str_replace(" ","_",$file['name']);
			$file['tmp_name'] = $resizedFile = str_replace(" ","_",$file['tmp_name']);
            $Media = "/uploads/$DomainFolder/media/$Rand" . $file['name'];
            if($file['name'] == ""){
                $Article_Code = $Article_Code;
            }else{
                $Article_Code = $Array['siteactual']['domain'] . "/uploads/$DomainFolder/media/$Rand" . $file['name'];
            }
...
function Cw_Img_Resize($Array,$file,$Sizes,$Name){
    ...
    if(is_array($Sizes)){
        foreach($Sizes as $key => $value){
            ...
            $resizedFile = "uploads/$DomainFolder/images/" . $Name . "-" . $value . ".png";
...
function Cw_Ecommerce_Default($Article,$Array){
    ...
    if(is_array($Array["userinfo"]["other"])){
        if($Array["userinfo"]["other"]["wholesale"] == "1"){
            $Article["content"]["prodprice"] = $Article["content"]["newprice"];
            $Article["content"]["newprice"] = $Article["other"]["wholesale"]["price"];
            if($Article["content"]["prodprice"] < $Article["content"]["newprice"] OR $Article["content"]["newprice"] == ""){
                $Article["content"]["newprice"] = $Article["content"]["prodprice"];
            }
        }
    }
...
////////// FETCH SUPPLIER RETURN POLICY \\\\\\\\\\\
    ...
    if(is_array($Article['other'])){
        if(is_array($Article['other']['resell'])){
...
// PROCESS FOR DB UPLOAD \\
    ...
    $Add = serialize($Add);
    $Del = serialize($Del);
    mysqli_query($CwDb,"INSERT INTO cw_alerts(name, other, webid, type, user, date, new, old, post) VALUES('$Name', '$Other', '$WebId', '$Type', '$User', '$Date', '$Add', '$Del', '$Info[id]') ")or die(mysql_error());
}
...
function CwFilterVidUrl($vid){
    if (strpos($vid, "youtube") !== false) {
        $Id = youtube_id_last_chance($vid);
        $vid = "https://www.youtube.com/embed/$Id";
        $Info["img"] = "http://img.youtube.com/vi/$Id/0.jpg";
        $Info["url"] = $vid;
    }else if (strpos($vid, "vimeo") !== false) {
        $Id = vimeo_id_last_chance($vid);
