...
class usersOnline {
	...
	*/
	...
	function new_user() {
		$insert = mysqli_query ($CwDb,"INSERT INTO useronline(timestamp, ip) VALUES ('$this->timestamp', '$this->ip')");
		if (!$insert) {
	...
	function delete_user() {
		$delete = mysqli_query ($CwDb,"DELETE FROM useronline WHERE timestamp < ($this->timestamp - $this->timeout)");
		if (!$delete) {
	...
	function count_users() {
		if (count($this->error) == 0) {
			$count = mysqli_num_rows ( mysqli_query($CwDb,"SELECT DISTINCT ip FROM useronline"));
			return $count;
		}
	}
}
