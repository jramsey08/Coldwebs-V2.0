...
if($content['dbcontrol'] == "1"){
    ...
    if(!isset($Website_Id)){$Website_Id = mysql_real_escape_string($content['websiteid']); }
    if(!isset($Website_Offline)){$Website_Offline = mysql_real_escape_string($content['offline']); }
    if(!isset($Site_Setup)){$Site_Setup = mysql_real_escape_string($content['sitesetup']); }
}
unset($content);

mysql_connect("$Website_Host", "$Website_Username", "$Website_Password") or die(mysql_error());
mysql_select_db("$Website_Database") or die(mysql_error());
